...
class VoiceGiftListener {
  ...
  async onVoiceStateUpdate([oldState, newState]: ArgsOf<"voiceStateUpdate">) {
    ...
    if (oldState.channel && !newState.channel) {
      ...
      try {
        await this.checkAndAwardGifts(userId);
      } catch (error) {
        logger.error(`Ошибка при проверке наград подарков для ${userId}: ${error}`);
      }
    }
  }
   ...
   */
  private async checkAndAwardGifts(userId: string): Promise<void> {
    try {
...
export default VoiceGiftListener;